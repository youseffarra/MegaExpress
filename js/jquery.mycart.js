  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h4 class="modal-title text-right" id="myModalLabel">عناصر السلة</h4>' +
        '</div>' +
        '<div class="modal-body">' +
        '<table class="table table-hover table-responsive" id="' + idCartTable + '"></table>' +
        '</div>' +
        '<div class="modal-footer">' +
        '<button type="button" class="btn btn-light" data-dismiss="modal">متابعة التسوق</button>' +
        '<button type="button" class="btn bg-purple text-white ' + classCheckoutCart + '">إتمام الدفع</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        $cartTable.append(
          '<tr class="text-right" title="' + this.summary + '" data-id="' + this.id + '" data-price="' + this.price + '">' +
      ...
      if (products.length && discountPrice !== null) {
        $cartTable.append(
          '<tr class="text-right" style="color: red">' +
          '<td></td>' +
          '<td><strong>المجموع (مع الحسم): </strong></td>' +
          '<td></td>' +
          '<td></td>' +
